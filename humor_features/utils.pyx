...
wcRatiosDf = pd.read_csv("data/ANC-token-count.csv", sep=',',encoding='ISO-8859-1',header=0).set_index("word")
writtenDf = pd.read_csv("data/ANC-written-count.csv", sep=',', encoding='ISO-8859-1',header=0)
writtenDf = writtenDf.set_index("Word")
writtenDf.drop(columns=["POS","Lemma"],inplace=True)
writtenDf = writtenDf.groupby("Word").sum()
spokenDf = pd.read_csv("data/ANC-written-count.csv", sep=',', encoding='ISO-8859-1',header=0)
